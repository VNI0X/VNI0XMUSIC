...
from VNI0XMUSIC.mongo.afkdb import LOGGERS as OWNERS
from VNI0XMUSIC.utils.database import add_served_chat, get_assistant


@app.on_message(filters.command("repo"))
async def help(client: Client, message: Message):
    await message.reply_photo(
        photo=f"https://files.catbox.moe/bd8iwy.webp",
        caption=f"""🍁 𝐏𝐀𝐇𝐋𝐄 𝐏𝐀𝐏𝐀 𝐁𝐎𝐋 🍁""",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "🍁 sᴏᴜʀᴄᴇ 🍁", url=f"https://t.me/HEROKU_CLUB"
                    )
                ]
            ]
        ),
    ...
    await message.reply_photo(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "🍁 sᴏᴜʀᴄᴇ 🍁", url=f"https://t.me/NOBITA_SUPPORT"
                    )
                ]
            ]
        ),
...
async def add_allbot(client, message):
    ...
    if len(command_parts) != 2:
        await message.reply(
            "**⚠️ ɪɴᴠᴀʟɪᴅ ᴄᴏᴍᴍᴀɴᴅ ғᴏʀᴍᴀᴛ. ᴘʟᴇᴀsᴇ ᴜsᴇ ʟɪᴋᴇ » `/gadd @NOBITA_SONG_BOT`**"
        )
        return
